...
function add_arcs() {
	while true; do (
		...
		( echo 'add-arcs:';
		  ...
		  done; echo "" ) ) | nc localhost $TCPPORT >/dev/null || exit 0
		sleep $(random 1 .01)
	done
}
...
function remove_arcs() {
	while true; do
		N=$( (echo 'use-graph test'; echo 'stats'; sleep .5) | nc localhost $TCPPORT | grep ArcCount | cut -d ',' -f 2 )
		if [[ x$N == x ]]; then N=100; fi
		if (( $N > 50000 )); then 
			N=$(( $N / 2 ))
			echo '		***' removing $N
		 	(echo 'authorize password fred:test'; echo 'use-graph test'; 
			 echo "list-by-head 0 $N > tmpout") | nc localhost $TCPPORT
			(echo 'authorize password fred:test'; 
			 echo 'use-graph test'
			 echo 'remove-arcs < tmpout'
			 echo stats
			 sleep 5
			) | nc localhost $TCPPORT || exit 0
			date +'%F %H:%M:%S'
			echo '====== server: ======'
